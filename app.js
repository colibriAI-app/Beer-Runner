...
const TXT = {
  fr: {
    ...
    futTitre: "Ton fût — crédit périssable", futExpire: "Demi-vie 24 h · perdues à 72 h — impossible de thésauriser !",
    futScience: "🔬 Une pinte apporte ~215 kcal, mais l'alcool est brûlé en priorité et bloque les graisses : facture réelle +20 %. Et le crédit périme : un déficit calorique ne se met pas en réserve d'avance.",
    welcomeTitre: "🍺 Bienvenue sur Beer Runner !",
    welcomeIntro: "Ici, on ne compte pas les calories — on les transforme en pintes ! Chaque course crédite des kcal dans ton fût, et chaque pinte bue coûte ce que tu as gagné en courant. Objectif : que chaque bière soit justifiée.",
    welcomeOnglets: "Les onglets :",
    welcomeAccueil: "🏠 Accueil — ton fût (crédit périssable), la bière du jour, tes stats et le bouton pour lancer une course.",
    welcomeRun: "🏃 Course — GPS ou simulation : la pinte se remplit en direct, les 10 premières minutes sont un échauffement, puis chaque pinte gagnée s'ajoute à ton fût.",
    welcomeBilan: "📊 Bilan — le détail de ta semaine : kcal nettes, pintes justifiées, delta de poids simulé (7 700 kcal ≈ 1 kg).",
    welcomeBrasseries: "🍺 Brasseries — 362 bières cultes : une nouvelle chaque jour avec son anecdote façon Jivay.",
    welcomeDefis: "🏁 Défis — les badges à débloquer et les défis de groupe entre amis.",
    welcomeConseil: "💡 À savoir : le crédit du fût se périme (demi-vie 24 h, perdu à 72 h), une conso < 12 h avant une course réduit tes gains de 12 %, et rester sobre les jours précédents te donne jusqu'à +10 %.",
    welcomeBtn: "C'est parti ! 🍺",
    gueuleBadge: "🍺 Gueule de bois : gains −12 % (conso < 12 h)",
    soberBadge: "🌱 Sobre {d} j : gains +{p} %",
    ...
    warmupInfo: "🔥 Encore {m} min d'échauffement — après, la 1ʳᵉ pinte se comptabilise !",
  ...
  en: {
    ...
    futTitre: "Your keg — perishable credit", futExpire: "Half-life 24h · lost after 72h — no stockpiling!",
    futScience: "🔬 A pint carries ~215 kcal, but alcohol gets burned first and blocks fat: real bill +20%. And the credit expires: a calorie deficit can't be banked in advance.",
    welcomeTitre: "🍺 Welcome to Beer Runner!",
    welcomeIntro: "Here we don't count calories — we turn them into pints! Every run credits kcal to your keg, and every pint you drink costs what you earned running. Goal: every beer justified.",
    welcomeOnglets: "The tabs:",
    welcomeAccueil: "🏠 Home — your keg (perishable credit), the beer of the day, your stats and the button to start a run.",
    welcomeRun: "🏃 Run — GPS or simulation: the pint fills live, the first 10 minutes are a warm-up, then every pint won goes into your keg.",
    welcomeBilan: "📊 Summary — your week in detail: net kcal, justified pints, simulated weight change (7,700 kcal ≈ 1 kg).",
    welcomeBrasseries: "🍺 Breweries — 362 cult beers: a new one every day with its Jivay-style anecdote.",
    welcomeDefis: "🏁 Challenges — badges to unlock and group challenges with friends.",
    welcomeConseil: "💡 Good to know: the keg credit expires (24h half-life, gone after 72h), drinking < 12h before a run cuts your gains by 12%, and staying sober earns you up to +10%.",
    welcomeBtn: "Let's go! 🍺",
    gueuleBadge: "🍺 Hangover: gains −12% (drank < 12h ago)",
    soberBadge: "🌱 Sober {d} d: gains +{p}%",
    ...
    warmupInfo: "🔥 {m} more min of warm-up — then your 1st pint starts counting!",
...
function App() {
  ...
  // modale de bienvenue : fermée uniquement à la main (bouton), rouvre à chaque connexion
  ...
  if (page === "home") {
    ...
    return (
      <div className={`${theme} min-h-[100dvh] overflow-y-auto bg-gray-950 text-white flex flex-col items-center gap-2.5 px-4 pt-3 pb-3 text-center`}>
        ...
        {/* ===== MODALE DE BIENVENUE — fermeture manuelle ===== */}
        {welcome && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4 overflow-y-auto">
            <div className="bg-gray-900 border-2 border-amber-500 rounded-3xl p-5 max-w-sm w-full my-6 text-left space-y-3">
              <h2 className="text-2xl font-black text-amber-400 text-center leading-tight">{T("welcomeTitre")}</h2>
              <p className="text-[13px] leading-snug text-white/85">{T("welcomeIntro")}</p>
              <div className="pt-1 border-t border-white/10">
                <p className="text-[11px] font-black text-amber-300 uppercase tracking-wide mb-1.5">{T("welcomeOnglets")}</p>
                <ul className="space-y-1.5">
                  {["welcomeAccueil", "welcomeRun", "welcomeBilan", "welcomeBrasseries", "welcomeDefis"].map((k) => (
                    <li key={k} className="text-[12px] leading-snug text-white/70">{T(k)}</li>
                  ))}
                </ul>
              </div>
              <p className="text-[12px] leading-snug text-white/60 pt-1 border-t border-white/10">{T("welcomeConseil")}</p>
        ...
        <div className="w-full max-w-md flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-sm font-black text-amber-300">{s.streak} 🔥 <span className="text-[12px] text-white/50 font-normal">{T("jours")}</span></span>
            ...
            return (
              <>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div><div className="text-lg font-black text-amber-300 leading-tight">{w.km.toFixed(1)}</div><div className="text-[11px] text-white/60">{T("kmWeek")}</div></div>
                  <div><div className="text-lg font-black text-amber-300 leading-tight">{w.pintes.toFixed(1)} 🍺</div><div className="text-[11px] text-white/60">{T("equiv")}</div></div>
                  <div><div className="text-lg font-black text-amber-300 leading-tight">{w.jours}</div><div className="text-[11px] text-white/60">{T("joursCourus")}</div></div>
                </div>
                <div>
                ...
                {r.meilleure && r.meilleure.km > 0 && (() => {
                  ...
                  return (
                    <div className={`text-[12px] rounded-xl px-2 py-1 ${battu ? "bg-green-500/15 text-green-300 font-bold" : "text-white/50"}`}>
                ...
                <div className="flex items-center gap-1.5 pt-0.5 flex-wrap">
                  <span className="text-[12px] text-white/50">🏅 {m.badges.length}/{BADGES.length}</span>
        ...
        {(() => {
          ...
          return (
            <div className="w-full max-w-md bg-white/5 rounded-2xl px-3 py-2 border border-white/10 text-left">
              ...
              <p className="text-[11px] leading-snug text-white/50 pt-1.5 border-t border-white/10 mt-1.5">{T("futScience")}</p>
            </div>
          );
        })()}
            ...
            <div className="w-full max-w-md bg-white/5 rounded-2xl px-3 py-2 border border-white/10 text-left">
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[12px] leading-snug text-white/60">{b.type} · {b.origine}</div>
                </div>
              </div>
        ...
        <div className="w-full max-w-md bg-white/5 rounded-2xl p-3 border border-white/10 space-y-2 text-left">
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-white/10">
            <div className="flex-1 min-w-0">
              <div className="text-[13px] font-bold text-white/80">{T("notifTitre")}</div>
              <p className="text-[11px] text-white/45 leading-snug">{T("notifInfo")}</p>
            </div>
            {typeof Notification !== "undefined" ? (
              <button onClick={async () => {
                ...
                className={`px-3 py-1.5 rounded-full text-[13px] font-black shrink-0 ${notifOn ? "bg-white/10 text-white/60" : "bg-amber-500 text-black"}`}>
                {notifOn ? T("notifOff") : T("notifBtn")}
              </button>
            ) : (
              <span className="text-[11px] text-white/40 shrink-0">{T("notifNo")}</span>
            )}
          </div>
        </div>
        ...
        <a href="mailto:ColibriAI.app@gmail.com"
          className="text-[11px] text-white/35 hover:text-amber-400/70 underline underline-offset-2">
          {T("footer")}
        </a>
      </div>
    );
